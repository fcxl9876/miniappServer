...
public class DateUtil {
	...
	public static final Map<TimeTypeEnum, DateField> DateFieldMap = new HashMap<>();
	static {
		DateFieldMap.put(TimeTypeEnum.day, DateField.HOUR);
		DateFieldMap.put(TimeTypeEnum.week, DateField.WEEK_OF_YEAR);
		DateFieldMap.put(TimeTypeEnum.month, DateField.DAY_OF_YEAR);
		DateFieldMap.put(TimeTypeEnum.year, DateField.MONTH);
	}
	/**
	 * 获取当前日期
	 * 
	 ...
	 * @return
	 */
	public static String dateToStringReport(Date date, TimeTypeEnum timeType) {
		String pattern = DATE_SMALL_YMDHH;
		if (TimeTypeEnum.day.compareTo(timeType) == 0){
			pattern = DATE_SMALL_YMDHH;
		} else if (TimeTypeEnum.hour.compareTo(timeType) == 0) {
			pattern = DATE_SMALL_YMDHH;
		} else if (TimeTypeEnum.month.compareTo(timeType) == 0) {
			pattern = DATE_SMALL_STR;
		} else if (TimeTypeEnum.year.compareTo(timeType) == 0) {
			pattern = DATE_SMALL_YM;
		}
	...
	public static Date stringToDate(String str, TimeTypeEnum timeType) {
		...
		switch (timeType){
			case realTime:
				date = stringToDate(str, DATE_FULL_STR);
				break;
			case hour:
				date = stringToDate(str, DATE_SMALL_YMDHH);
				if (ObjectUtil.isNull(date)){
					date = stringToDate(str, DATE_SMALL_YMDH);
				}
				if (ObjectUtil.isNull(date)){
					date = stringToDate(str, DATE_SMALL_YMDHMS);
				}
				break;
			case week:
			case day:
				date = stringToDate(str, DATE_SMALL_STR);
				if (ObjectUtil.isNull(date)){
					date = stringToDate(str, DATE_FULL_STR);
				}
				break;
			case month:
			case quarter:
				date = stringToDate(str, DATE_SMALL_YM);
				if (ObjectUtil.isNull(date)){
					date = stringToDate(str, DATE_SMALL_STR);
				}
				break;
			case year:
				date = stringToDate(str, DATE_SMALL_Y);
				if (ObjectUtil.isNull(date)){
					date = stringToDate(str, DATE_SMALL_YM);
				}
				break;
	...
	public static List<String> getTimeList(String d1, String d2, TimeTypeEnum timeType){
		Date t1 = stringToDate(d1, timeType);
		Date t2 = stringToDate(getEndTimeByType(stringToDate(d2, timeType), timeType, DATE_FULL_STR), DATE_FULL_STR);

		List<DateTime> dateTimes = cn.hutool.core.date.DateUtil.rangeToList(t1, t2, DateFieldMap.get(timeType));
		if (timeType.compareTo(TimeTypeEnum.week) == 0){
			return dateTimes.stream().map(l -> {
				Calendar calendar = l.toCalendar();
				calendar.setFirstDayOfWeek(Calendar.MONDAY);
				int week = calendar.get(Calendar.WEEK_OF_YEAR);
				int year = calendar.get(Calendar.YEAR);
				return "第" + year +"年第" + week + "周";
			}).collect(Collectors.toList());
		} else {
			return dateTimes.stream().map(l ->
					dateToStringReport(l.toJdkDate(), timeType)).collect(Collectors.toList());
		}
	...
	public static String getStartTimeByType(String time, TimeTypeEnum timeType, String pattern){
		return getStartTimeByType(stringToDate(time, timeType), timeType, pattern);
	}

	public static String getEndTimeByType(String time, TimeTypeEnum timeType, String pattern){
		return getEndTimeByType(stringToDate(time, timeType), timeType, pattern);
	}
	...
	public static String getEndTimeByType(Date date, TimeTypeEnum timeType, String pattern){
		...
		calendar.setTime(Objects.requireNonNull(stringToDate(getStartTimeByType(calendar.getTime(), timeType, pattern), pattern)));
	...
	public static void main(String[] args) {
		System.out.println(getTimeList("2022", "2023", TimeTypeEnum.year));
	}
